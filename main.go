...
func (tm *TaskManager) tasksHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		if err != nil {
			...
			json.NewEncoder(w).Encode(map[string]string{"error": "Внутрення ошибка сервера"})
			return
		}
		...
		fmt.Fprint(w, newTask.ID, "\n")
		fmt.Fprint(w, newTask.Path, "\n")
		//	case http.MethodGet
...
func (tm TaskManager) addURLHandler(w http.ResponseWriter, r *http.Request) {
	...
	if len(vars)+len(tm.tasks[taskID].Files) >= 3 {
		w.Header().Set("Content-type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
	...
	tm.tasks[taskID].Files = append(tm.tasks[taskID].Files, data.URL)
	if len(tm.tasks[taskID].Files) == 3 {
		tm.createdArchive(taskID)
	}
}

func (tm TaskManager) createdArchive(taskID uuid.UUID) {
